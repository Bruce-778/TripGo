...
#### 第一步：解锁开发者模式
1. 访问：`http://localhost:3000/dev` 或 `http://localhost:3001/dev`
2. 输入你的 `ADMIN_TOKEN`（在 `.env` 文件中设置）
3. 点击"解锁开发者模式"
4. 解锁成功后，导航栏会显示"管理后台"链接

#### 第二步：进入管理后台
1. 点击导航栏的"管理后台"链接，或直接访问：`http://localhost:3000/admin`
2. 再次输入 `ADMIN_TOKEN` 进行登录
3. 进入订单管理界面
...
- 后台解锁：`http://localhost:3000/dev`
- 管理后台：`http://localhost:3000/admin`（需先解锁）
...
### Q: 如何重置 ADMIN_TOKEN？
A: 修改 `.env` 文件中的 `ADMIN_TOKEN`，然后重启服务器。
